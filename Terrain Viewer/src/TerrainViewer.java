...
public class TerrainViewer extends Frame implements MouseMotionListener {
	...
	//================================================================================================================
	//
	//                                   generate view with 90 degree FOV
	//
	//================================================================================================================
	
	public static void generateFOVView(float[][] heightMap, int xPosition, int yPosition, float distancePerPixel, int screenWidth, int screenHeight, String fileName) {
		float[][] relativeHeightMap = relativeHeight(heightMap, xPosition, yPosition, distancePerPixel);
		BufferedImage outputScreen = new BufferedImage(screenWidth, screenHeight, BufferedImage.TYPE_INT_RGB);
		
		int maxViewRange = yPosition;
		assert (screenWidth>maxViewRange*2);
		int viewRange = yPosition;
		for(int i=0; i<yPosition; i++) {
			float screenX = 0;
			for(int j=xPosition-viewRange; j<xPosition+viewRange; j++) {
				for(int k=0;k<(screenWidth/(viewRange*2+1));k++) {
					if(k>=relativeHeightMap[0].length || j<0 || j>=relativeHeightMap.length) {
						break;
					}
					if((int)screenX+k<screenWidth || (int)screenX+k>screenWidth) {
						
					}else if(screenHeight-(((int) relativeHeightMap[j][k])*screenHeight)<screenHeight) {
						
					}else if(screenHeight-(((int) relativeHeightMap[j][k])*screenHeight)>screenHeight) {
						outputScreen.setRGB((int)screenX+k, 0, new Color(0,255,0).getRGB());
					}else {
						outputScreen.setRGB((int)screenX+k, screenHeight-(((int) relativeHeightMap[j][k])*screenHeight), new Color(0,255,0).getRGB());
					}
				}
				screenX+=(screenWidth/((viewRange*2)+1));
			}
		}
		
		File outputFile = new File(fileName + ".png");
		try {
			ImageIO.write(outputScreen, "png", outputFile);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		...
		generateFOVView(heightMap, xPosition, yPosition, distancePerPixel, screenWidth, screenHeight, FOVFileName);
		
		timeTaken = System.currentTimeMillis() - startTime;
		System.out.println("FOV complete: " + timeTaken + " milliseconds");
		startTime = System.currentTimeMillis();
		...
		*/
		...
		//float[][] relativeHeightMap = relativeHeight(heightMap, xPosition, yPosition, distancePerPixel);
		/*for(int i=0; i<noScaleHeightMap.length; i++) {
			for(int j=0; j<noScaleHeightMap[0].length; j++) {
				System.out.println(noScaleHeightMap[i][j]);
			}
		}*/
		
		/*System.out.println(relativeHeightMap[220][205]);
		System.out.println(relativeHeightMap[180][70]);
		System.out.println(relativeHeightMap[345][295]);
		System.out.println(relativeHeightMap[285][530]);*/
		
		//float[] hsb = Color.RGBtoHSB(220,248,135,null);
		//System.out.println(hsb[0] + " " + hsb[1]);
